<?php
/**
 * Caldera Forms Zoho processor - contact config template
 *
 * @package   cf_zoho
...
if( !empty( $config_object['main']['token'] ) ){
	...
	if( !empty($fields['error'] ) ){
		echo '<div class="error"><p>Got an error: ' . $fields['error'] .' - Please check plugin is setup. (fields)</p></div>';
		return;
	}
	$zoho_users = $connect->get_users();
	if( is_string( $zoho_users ) ){
		echo '<div class="error"><p>Got an error: ' . $zoho_users .' - Please check plugin is setup. (user)</p></div>';
		return;
	}	
	$users = array();
	if ( ! empty( $zoho_users ) ) {
...
if( !empty( $config_object['fields']['contacts_fields'] ) ){
	$force_text_input = explode( "\n", $config_object['fields']['contacts_fields'] );
}
...
<div class="caldera-config-group">
	...
	<div class="caldera-config-field">
		<label><input type="checkbox" id="{{_id}}_approval_mode" class="field-config" name="{{_name}}[_approval_mode]" {{#if _approval_mode}}checked="checked"{{/if}}> <?php esc_html_e( 'Approval Mode', 'cf-zoho' ); ?></label>
		<label><input type="checkbox" id="{{_id}}_workflow_mode" class="field-config" name="{{_name}}[_workflow_mode]" {{#if _workflow_mode}}checked="checked"{{/if}}> <?php esc_html_e( 'Workflow Mode', 'cf-zoho' ); ?></label>
	</div>

	<div class="caldera-config-field">
		<label><input type="radio" id="{{_id}}_allow_duplicates" class="field-config" value="allow" name="{{_name}}[_allow_duplicates]" {{#is _allow_duplicates value=""}}checked="checked"{{/is}} {{#is _allow_duplicates value="allow"}}selected="selected"{{/is}}> <?php esc_html_e( 'Allow duplicate records', 'cf-zoho' ); ?></label>
		<label><input type="radio" id="{{_id}}_update_existing" class="field-config" value="update" name="{{_name}}[_allow_duplicates]" {{#is _allow_duplicates value="update"}}checked="checked"{{/is}}> <?php esc_html_e( 'Update existing records', 'cf-zoho' ); ?></label>
		<label><input type="radio" id="{{_id}}_no_updates" class="field-config" value="none" name="{{_name}}[_allow_duplicates]" {{#is _allow_duplicates value="none"}}checked="checked"{{/is}}> <?php esc_html_e( 'No duplicates, no updates', 'cf-zoho' ); ?></label>
	</div>
</div>
...
<div id="{{_id}}_contact">
	<?php
	foreach( $fields['contact']['section'] as $section ){
	...
	<?php foreach( $section['FL'] as $field_num=>$field ){ 
		...
		if( strtolower( $field['type'] ) == 'lookup' ){
			$field['val'] = $users;		
		}
		...
		if( ! empty( $field['val'] ) && ! empty( $force_text_input ) && ! in_array( $field['label'], $force_text_input ) ){
			...
			<div class="caldera-config-group">
				...
				<?php }else{ ?>
					<input type="text" id="{{_id}}<?php echo $field_num; ?>" class="field-config block-input magic-tag-enabled <?php if( $field['req'] == 'true'){ ?>required<?php } ?>" name="{{_name}}[<?php echo $key; ?>]" value="{{<?php echo $key; ?>}}">
				<?php } ?>
				</div>
			</div>
			<?php 
